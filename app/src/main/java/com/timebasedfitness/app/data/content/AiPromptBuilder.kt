...
object AiPromptBuilder {
    fun build(request: AiPlanRequest): String = """
        ...
        After the questions are answered, return ONLY valid JSON matching this exact schema. Do not wrap it in Markdown fences. Use categories only from MORNING, MEALS, WORKOUT, EVENING. Times must use 24-hour HH:mm format. Each category needs at least one step. For WORKOUT, use the optional days object when training varies by weekday; keys must be MONDAY through SUNDAY and values are that day's checklist.
          ...
          "categories": [
            {
              ...
              "days": {
                "MONDAY": ["Warm up", "Strength session"],
                "WEDNESDAY": ["Warm up", "Mobility session"]
              }
            }
          ]
        }
