...
class WidgetSnapshotTest {
    ...
    fun computeFromSelections_duringActiveWindow_showsActiveCategory() {
        ...
        val state = WidgetSnapshot.computeFromSelections(selections, now)

        assert(state is WidgetState.Active)
        assertEquals(Category.MORNING, (state as WidgetState.Active).category)
    }
    ...
    fun computeFromSelections_beforeNextWindow_showsNextUpcomingCategory() {
        ...
        assert(state is WidgetState.Next)
        assertEquals(Category.WORKOUT, (state as WidgetState.Next).category)
    }
    ...
    fun computeFromSelections_whenNoCategoriesEnabled_showsNothingScheduled() {
        ...
        val state = WidgetSnapshot.computeFromSelections(disabledSelections, now)

        assert(state is WidgetState.Idle)
    }
}
